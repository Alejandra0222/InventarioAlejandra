function guardarCategoria() { 
    ...
    .then(data => {
        ...
        mensajeCategoria.textContent = data.success ? "Categoria registrada con exito" : "Error: " + data.message;
        mensajeCategoria.style.display = "block"; 

        if (data.success) {
            setTimeout(() => window.location.href = "../html/categoria.html", 2000);
        }
    })
...
function cargarCategoria() {
    fetch("../php/categoria.php?action=obtenerCategoria")
    .then(response => response.json())
    .then(data => {
        if (data.success) {
